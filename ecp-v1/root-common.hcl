...
%{if contains(
  ["ado-mpool", "az-devcenter", "az-launchpad-main", "az-launchpad-backend", "az-launchpad-network"],
  regexall("^.*/(.+?)$", get_terragrunt_dir()
)[0][0])}
provider "azurerm" {
  alias  = "launchpad"
...
%{if contains(
  ["ado-mpool", "az-devcenter", "az-launchpad-backend", "az-launchpad-network", "az-launchpad-main"],
  regexall("^.*/(.+?)$", get_terragrunt_dir()
)[0][0])}
    azurerm = {
